# DIAMAS Registry – Custom Drupal Modules

This repository contains custom Drupal modules used by the DIAMAS Registry (https://registry.diamas.org/).
They target Drupal 9/10 and implement registry-specific UX, validation, and integration features.
...
### 2) email_protect
**Purpose:** Adds a computed “Protected Email” for **Organisation** nodes by obfuscating `field_ipsp_contact_email` (replaces `@` with `(at)`) to deter simple scraping.
...
**Purpose:** Synchronizes **organisation** node moderation state with **linked users** (via `field_ipsp_organisation_ref`), keeping visibility/access aligned with the organisation’s status.
...
### 5) webform_geonames
**Purpose:** Adds a **city autocomplete** to Drupal Webforms backed by **GeoNames** (and `restcountries.com` for country-code lookups).
